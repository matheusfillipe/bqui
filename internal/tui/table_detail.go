...
func (m TableDetailModel) renderSchemaField(field *bigquery.Column, indent int) string {
	indentStr := strings.Repeat("  ", indent)
	
	typeStr := DataTypeStyle.Render(string(field.Type))
	modeStr := ""
	if field.Required {
		modeStr = SubtleItemStyle.Render(" (REQUIRED)")
	} else if field.Repeated {
		modeStr = SubtleItemStyle.Render(" (REPEATED)")
	}

	line := fmt.Sprintf("%s%s %s%s", indentStr, ItemStyle.Render(field.Name), typeStr, modeStr)
	
	if field.Description != "" {
		line += SubtleItemStyle.Render(fmt.Sprintf(" - %s", field.Description))
	}

	result := line
	for _, subField := range field.Fields {
		result += "\n" + m.renderSchemaField(subField, indent+1)
	}
